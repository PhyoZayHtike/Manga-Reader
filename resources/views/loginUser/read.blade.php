     ...
     <div class="container-fluid">
        <div class="row">
           ...
           <div class="col-12 p-2 col-md-11 offset-md-1 mt-2">
            <a class="text-decoration-none hoverBlue" href="{{route('loginUser#home')}}">Home</a> / <a  class="text-decoration-none hoverBlue" href="">{{$data->title}}</a> / <a  class="text-decoration-none hoverBlue" href="">Chapter {{$chapterNum->chapter_number}}</a>
          </div>
                  <!-- drop drown -->
            ...
            <div class="btn-group">
                ...
                @foreach ($allChapters as $c)
                    @if ($c->chapter_number !== $previousChapter)
                                <li><a class="dropdown-item" href="{{ route('loginUser#view', ['chapterId' => $data->id, 'chapterNumber' => $c->chapter_number]) }}">
             ...
             <div class="">
                @if ($prevChapter)
                <a href="{{route('loginUser#view',['chapterId' => $data->id, 'chapterNumber' => $chapterNum->chapter_number -1])}}"><button class="btn btn-primary fw-semibold rounded-1 py-2"><i class="fa-solid fa-arrow-left"></i> Prev</button></a>
              @endif
              @if ($nextChapter)
                <a href="{{route('loginUser#view',['chapterId' => $data->id, 'chapterNumber' => $chapterNum->chapter_number +1])}}"><button class="btn btn-primary fw-semibold rounded-1 py-2">Next <i class="fa-solid fa-arrow-right"></i></button></a>
              @endif
             </div>
          </div>
        </div>
