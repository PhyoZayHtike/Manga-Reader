...
@section('main')
      ...
      <div class="container">
        <div class="mt-3">
            <a class="text-decoration-none hoverBlue" href="{{route('loginUser#home')}}">Home</a> / <a  class="text-decoration-none hoverBlue" href="">{{$detail->genre_name}}</a> / <a  class="text-decoration-none hoverBlue" href="">{{$detail->title}}</a>
        </div>
        ...
        <div class="row">
               ...
               <span class="">{{$detail->rating}}</span>
                ...
                <div class="row mt-lg-5">
                    ...
                    <div class="d-flex justify-content-around justify-content-lg-start mt-4">
                        @if (count($chapters) != 0)
                        <a class="btn btn-outline-dark btn-warning fw-bold text-white" href="{{ route('loginUser#view', ['chapterId' => $detail->id, 'chapterNumber' => $chapters->first()->chapter_number]) }}">
                            Read First
                        </a>
                        <a class="btn btn-outline-dark btn-warning fw-bold text-white ms-2" href="{{ route('loginUser#view', ['chapterId' => $detail->id, 'chapterNumber' => $chapters->last()->chapter_number]) }}">
                            Read Last
                        </a>
                        @endif
                    </div>
            ...
            <div class="row mt-4 p-3">
              ...
              <div class="col-2 mt-2">
                <form action="{{ route('loginUser#detail', $detail->id) }}" method="GET">
            ...
            @foreach ($chapters as $chapter)
                @if ($chapter->chapter_number !== $previousChapter)
                    ...
                    <a class="text-decoration-none text-info-emphasis" href="{{ route('loginUser#view', ['chapterId' => $detail->id, 'chapterNumber' => $chapter->chapter_number]) }}">
